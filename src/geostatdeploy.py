...
function generateLeafletPopup(feature, layer){
    ...
    for(prop in feature.properties){
        ...
        if(Array.isArray(feature.properties[prop]) && feature.properties[prop].length>1){
            ...
            if(feature.properties[prop].toString().startsWith("http")){
                popup+="<a href=\\""+feature.properties[prop]+"\\" target=\\"_blank\\">"+feature.properties[prop]+"</a>"
            }else if(feature.properties[prop].toString().includes("@")){
                popup+="<a href=\\"mailto:"+feature.properties[prop]+"\\" target=\\"_blank\\">"+feature.properties[prop]+"</a>"
            }else{
    ...
    if ".shp" in file or ".geojson" in file:
        ...
        for feat in res["features"]:
            ...
            if "id" not in feat and "id" in feat["properties"]:
                ...
                newf[-1]["id"]=feat["properties"]["ID"]
            elif "id" not in feat and "fid" in feat["properties"]:
                newf[-1]["id"]=feat["properties"]["fid"]
        ...
        with open(outpath + "/collections/" + fileid + "/items/indexc.html", 'w', encoding="utf-8") as f:
            ...
            for feat in geodict["features"]:
                ...
                for prop in feat["properties"]:
                    thevalue=str(feat["properties"][prop]).strip()
                    if thevalue.startswith("http"):
                        f.write("<td><a href=\""+thevalue+"\" target=\"_blank\">"+thevalue+"</a></td>")
                    elif thevalue.startswith("www."):
                        f.write("<td><a href=\"https://"+thevalue+"\" target=\"_blank\">"+thevalue+"</a></td>")
                    elif "@" in thevalue:
                        f.write("<td><a href=\"mailto:"+thevalue+"\">"+thevalue+"</a></td>")
                    else:
                        f.write("<td>"+thevalue+"</td>")
                f.write("</tr>")
                ccounter+=1
            f.write("</tbody></table>")    
...
with open(outpath + "/indexc.html", 'w', encoding="utf-8") as f:
    f.write(indexhtml)
